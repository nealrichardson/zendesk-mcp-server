# Zendesk MCP Server (Python)

A Python implementation of the Model Context Protocol (MCP) server for Zendesk API.
...
```bash
uvx --from "git+https://github.com/nealrichardson/zendesk-mcp-server#subdirectory=python" zendesk-mcp
```
...
```json
{
  "mcpServers": {
    "zendesk": {
      "command": "uv",
      "args": ["run", "--directory", "/path/to/zendesk-mcp-server/python", "python", "-m", "zendesk_mcp"]
    }
  }
}
```
